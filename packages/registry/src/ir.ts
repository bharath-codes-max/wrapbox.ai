...
export const ACTION_VERBS = ["read", "write", "delete", "execute", "query", "push", "merge", "invoke", "disclose", "transact", "configure", "connect", "list"] as const;
export const RESOURCE_TYPES = ["file", "folder", "repo", "branch", "database", "dataset", "schema", "table", "column", "db_statement", "endpoint", "api", "mcp_tool", "account", "cloud_resource", "iam_role", "payment_method", "payment_transaction", "saas_object", "any"] as const;
export const DECISIONS = ["ALLOW", "CONSTRAIN", "REVIEW", "BLOCK"] as const;
export const CONFIDENCES = ["low", "medium", "high"] as const;
export const ON_UNSUPPORTED = ["hold_activation", "block", "review", "accept_risk"] as const;
export const INVARIANTS = ["fail_closed_transform", "fail_closed_no_rule", "fail_closed_uninspected"] as const;
export const SUBJECT_KINDS = ["any", "group", "user", "agent_class", "device", "workload"] as const;
export const DEST_CLASS_NAMES = [...DESTINATION_CLASSES, ...Object.keys(CLASS_GROUPS)] as const;
...
export function extractionJsonSchema(): Record<string, unknown> {
  const raw = z.toJSONSchema(SurfaceIRSchema, { target: "draft-2020-12", unrepresentable: "any" }) as Record<string, unknown>;
  const walk = (node: unknown): void => {
    ...
    if (o.$defs && typeof o.$defs === "object") for (const v of Object.values(o.$defs as Record<string, unknown>)) walk(v);
    // Not part of the Structured Outputs subset.
    delete o.$schema; delete o.minimum; delete o.maximum; delete o.minLength; delete o.maxLength; delete o.pattern; delete o.format; delete o.default;
  };
  walk(raw);
  return raw;
}

export function severityTier(d: (typeof DECISIONS)[number]): number {
...
export function specificity(c: Pick<ClauseIR, "destination" | "data" | "resource" | "scope" | "conditions" | "subject">): number {
  let n = 0;
  const d = c.destination;
  if (d && (d.hosts?.length || d.services?.length || d.groups?.length || d.notIn || d.classes?.length)) n += 2;
  if (c.data?.length) n += 2;
  if (c.resource.paths?.include?.length || c.resource.refs?.length) n += 1;
  if (c.resource.type && c.resource.type !== "any") n += 1;
  if (c.scope?.environments?.length) n += 1;
  if (c.conditions?.requires?.length) n += 1;
  if (c.subject.kind !== "any") n += 1;
  return n;
}

export function derivePriority(c: Pick<ClauseIR, "decision" | "destination" | "data" | "resource" | "scope" | "conditions" | "subject" | "catchAll">): number {
  if (c.catchAll) return 10;
