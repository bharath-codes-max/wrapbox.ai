...
import { dataTypes, CONFIDENCE_RANK, type Confidence } from "./datatypes.js";
import { detectorRegistry, covers, type DetectorAvailability } from "./detectors.js";
import type { ExtractorAvailability } from "./extractors.js";
import { canExecute, type TransformAvailability } from "./transforms.js";
import { expandClass, type DestinationClass } from "./destinations.js";
...
export interface Requirement {
  capability: string;        // e.g. "content.PII.CONTACT.EMAIL@medium", "transform.REVERSIBLE_TOKENIZE.PII.CONTACT.EMAIL", "destination.class", "parse.docx"
  facet: Facet;
  purpose: string;
...
export interface RuntimeSnapshot {
  ...
  observes: string[];
  identity: Array<{ capability: string; proven: boolean }>;
  transport: {
  ...
  registryVersions: { dataTypes: string; destinations: string; detectors: string; transforms: string; extractors: string };
}
...
export function formatsFor(clause: ClauseIR): string[] {
  ...
  for (const e of exts) {
    ...
    else if (["csv"].includes(e)) wanted.add("csv");
    else if (["tsv"].includes(e)) wanted.add("tsv");
    else if (["html", "htm"].includes(e)) wanted.add("html");
    else if (["yaml", "yml"].includes(e)) wanted.add("yaml");
    else if (["xml"].includes(e)) wanted.add("xml");
  ...
  if (clause.data?.length && clause.decision !== "ALLOW") { wanted.add("csv"); wanted.add("docx"); wanted.add("xlsx"); wanted.add("pdf"); }
  return [...wanted];
}

export function requirements(clause: ClauseIR): Requirement[] {
  const reqs: Requirement[] = [];
  const R = (capability: string, facet: Facet, purpose: string, softenable = false) => reqs.push({ capability, facet, purpose, softenable });

  // Identity — proven subjects only; a device-only runtime enforces "employees" on the whole device (DEGRADED).
  if (clause.subject.kind !== "any") R(`identity.${clause.subject.kind}`, "identity", `verify the subject (${clause.subject.kind})`, true);

  // Destination.
  const d = clause.destination;
  if (d) {
    if (d.classes?.length || d.notIn?.classes?.length) R("destination.class", "destination", "classify the destination host");
    if (d.hosts?.length || d.services?.length || d.groups?.length || d.notIn?.hosts?.length || d.notIn?.services?.length || d.notIn?.groups?.length) R("destination.host", "destination", "observe the destination host");
    if (d.unresolved?.length) R(`destination.unresolved`, "destination", `resolve “${d.unresolved.join(", ")}” to addresses`, true);
    if (d.trust && !d.classes?.length && !d.hosts?.length && !d.services?.length && !d.groups?.length && !d.notIn) R("destination.class", "destination", `classify the destination as ${d.trust}`);
  }
  ...
  for (const ref of clause.data ?? []) {
    ...
    if (ref.owner) R(`provenance.${ref.owner}`, "data", `prove the data belongs to ${ref.owner}`, true);
  }

  // Parse — every format the resource may arrive in must have an available extractor.
  if (clause.data?.length) for (const f of formatsFor(clause)) R(`parse.${f}`, "parse", `read ${f} content`, f !== "text" && f !== "json" && f !== "multipart");

  // Transform — CONSTRAIN must be executable for each targeted type.
  if (clause.decision === "CONSTRAIN") {
    const ts = clause.transforms ?? [];
    if (!ts.length) R("transform.none", "transform", "a CONSTRAIN names no transform");
    for (const t of ts) {
      const targets = t.targets === "matched" ? (clause.data ?? []).map((r) => r.type) : (t.targets.types ?? (clause.data ?? []).map((r) => r.type));
      if (!targets.length && t.targets !== "matched" && t.targets.fields?.length) R(`transform.${t.handler}.fields`, "transform", `apply ${t.handler} to named fields`);
      for (const ty of targets) R(`transform.${t.handler}.${ty}`, "transform", `apply ${t.handler} to ${ty}`);
    }
  }

  if (clause.decision === "REVIEW") R("review.hold", "review", "hold the request until a human decides");

  // Authored predicates need their fields observed.
  for (const p of clause.conditions?.requires ?? []) R(`observe.${p.field}`, "resource", `evaluate ${p.field}`);

  // Transport: a network protection is only as complete as the transports observed.
  if (clause.decision !== "ALLOW") R("transport.observed", "transport", "the request must traverse a path Wrapbox controls", true);
  return reqs;
}
...
export interface CoverageVerdict {
  ...
  met: Requirement[];
  unmet: Array<{ req: Requirement; reason: string }>;
...
export function coverage(clause: ClauseIR, rt: RuntimeSnapshot | null): CoverageVerdict {
  const reqs = requirements(clause);
  const met: Requirement[] = [];
  const unmet: CoverageVerdict["unmet"] = [];
  const notes: string[] = [];
  const unobservableTypes: string[] = [];
  const untransformableTypes: string[] = [];
  const reg = dataTypes();
  ...
  for (const req of reqs) {
    ...
    if (head === "identity") { ok = identity.get(req.capability) === true; reason = ok ? "" : "no trusted identity on this plane"; }
    else if (head === "destination") {
      if (req.capability === "destination.class") { ok = observes.has("tool_input.destination_class"); reason = ok ? "" : "runtime does not classify destinations"; }
      else if (req.capability === "destination.host") { ok = observes.has("tool_input.host"); reason = ok ? "" : "runtime does not observe the host"; }
      else { ok = false; reason = req.purpose; }
    }
    else if (head === "content") {
      const [typeWithConf] = [rest.join(".")];
      const [type, conf] = typeWithConf.split("@") as [string, Confidence];
      const known = reg.has(type) || reg.lineage(type).some((a) => reg.has(a));
      if (!known || type.startsWith("CUSTOM.") && !reg.has(type)) { ok = false; reason = `“${type}” is not a registered data type — register it (tenant type) before it can be enforced`; }
      else {
    ...
    else if (head === "provenance") { ok = availDet.some((d) => d.id === "wrapbox.edm" || d.id === "wrapbox.label.mip"); reason = ok ? "" : "no provenance detector (EDM index or label map) is configured"; }
    else if (head === "parse") {
      const f = rest.join(".");
      ...
      if (req.capability === "transform.none") { ok = false; reason = "CONSTRAIN names no transform"; }
      else {
        const handler = rest[0] as import("./datatypes.js").TransformId;
        const target = rest.slice(1).join(".");
        if (target === "fields") { ok = rt.transforms.some((t) => t.handler === handler); reason = ok ? "" : `runtime cannot execute ${handler}`; }
        else {
          const support = reg.transformSupport(target, handler);
          if (support !== "supported") { ok = false; reason = `${handler} is ${support === "unsafe" ? "declared unsafe" : "not applicable"} for ${target} — the runtime will block rather than forward`; untransformableTypes.push(target); }
          else { ok = canExecute(rt.transforms, handler, target); if (!ok) { reason = `runtime does not implement ${handler} for ${target}`; untransformableTypes.push(target); } }
        }
      }
    }
    else if (head === "review") { ok = true; }
    else if (head === "observe") { ok = observes.has(rest.join(".")); reason = ok ? "" : `runtime does not observe ${rest.join(".")}`; }
    else if (head === "transport") {
      ok = true;
      if (!rt.transport.ports.includes(443)) { ok = false; reason = "HTTPS is not captured"; }
      if (!rt.transport.plaintextHttp) notes.push("plain HTTP (port 80) is not observed by this plane");
      if (rt.transport.ports.length && !rt.transport.ports.includes(8443)) notes.push(`only TCP ports ${rt.transport.ports.join(", ")} are captured; HTTPS on other ports is not observed`);
      if (rt.transport.websocket === "bypassed") { ok = false; reason = "WebSocket upgrades bypass inspection"; }
      // Never-decrypt classes inside the clause's destination scope.
      const classesInScope = new Set<DestinationClass>();
      for (const c of clause.destination?.classes ?? []) for (const x of expandClass(c)) classesInScope.add(x);
      if (!clause.destination) for (const x of ["APPROVED_AI", "KNOWN_AI_UNAPPROVED", "APPROVED_SAAS", "PARTNER", "GENERIC_EXTERNAL", "UNKNOWN_EXTERNAL", "INTERNAL", "PERSONAL_EXEMPT", "INFRA_EXEMPT"] as DestinationClass[]) classesInScope.add(x);
      const nd = rt.transport.neverDecrypt.filter((c) => classesInScope.has(c));
      if (nd.length) notes.push(`destinations classed ${nd.join(", ")} are never decrypted (host-level decision only) — edit the exemption list to change this`);
    }
    else { ok = false; reason = `unknown capability ${req.capability}`; }

    if (ok) met.push(req); else unmet.push({ req, reason });
  }
  ...
  if (hard.some((u) => u.req.capability.startsWith("content.") && u.reason.includes("not a registered data type"))) status = "pending";
  else if (hard.length) status = "understood_only";
  else if (unmet.length) status = "degraded";
  else status = "enforced";

  // A protection over data must have SOMETHING observable to pin on, else it can never fire.
  if (status === "enforced" && clause.data?.length && unobservableTypes.length === (clause.data?.length ?? 0)) status = "understood_only";
  return { status, met, unmet, notes, unobservableTypes, untransformableTypes };
}

export interface ActivationReport {
  ok: boolean;
  blockers: Array<{ clauseId: string; code: string; message: string }>;
  warnings: Array<{ clauseId: string; message: string }>;
  perClause: Record<string, CoverageVerdict>;
}

/** The activation invariant (I1/I2). Pure; the caller decides what to do with the report. */
export function checkActivation(ir: PolicyIR, rt: RuntimeSnapshot | null): ActivationReport {
  const blockers: ActivationReport["blockers"] = [];
  const warnings: ActivationReport["warnings"] = [];
  const perClause: Record<string, CoverageVerdict> = {};
  ...
  for (const c of ir.clauses) {
    ...
    if (protection && (v.status === "understood_only" || v.status === "pending")) {
      ...
      if (c.onUnsupported === "hold_activation") {
        ...
        if (!c.acceptRisk?.by || !c.acceptRisk?.reason) blockers.push({ clauseId: c.id, code: "accept_risk_unsigned", message: "accept_risk requires an identity and a reason; both are recorded in evidence." });
        else warnings.push({ clauseId: c.id, message: `risk accepted by ${c.acceptRisk.by}: ${c.acceptRisk.reason} (${why})` });
      } else {
    ...
    if (protection && v.status === "degraded") warnings.push({ clauseId: c.id, message: `enforced on what Wrapbox observes; gaps: ${v.unmet.map((u) => u.reason).concat(v.notes).join("; ")}` });
  }
