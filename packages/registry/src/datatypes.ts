...
export interface DataType {
  ...
  defaults: { minCount: number; minConfidence: Confidence; bulkCount?: number };
...
const STD_EVIDENCE: DataType["evidence"] = ["label", "count", "confidence", "detector", "detector_version", "unit_path", "field_names"];
const TOKENIZABLE: DataType["transforms"] = { REVERSIBLE_TOKENIZE: "supported", REDACT: "supported", MASK: "supported", HASH: "supported", DROP_FIELD: "supported", FORMAT_PRESERVING: "unsafe" };
const REDACT_ONLY: DataType["transforms"] = { REDACT: "supported", DROP_FIELD: "supported", REVERSIBLE_TOKENIZE: "unsafe", MASK: "unsafe" };
const NO_TRANSFORM: DataType["transforms"] = { REDACT: "not_applicable", MASK: "not_applicable", REVERSIBLE_TOKENIZE: "not_applicable", HASH: "not_applicable", DROP_FIELD: "supported" };
const ALL_ACTIONS: Decision[] = ["ALLOW", "CONSTRAIN", "REVIEW", "BLOCK"];
const NO_CONSTRAIN: Decision[] = ["ALLOW", "REVIEW", "BLOCK"];

function t(
  id: string, label: string, aliases: string[],
  o: Partial<Omit<DataType, "id" | "label" | "aliases">> = {},
): DataType {
...
export const BUILTIN_TYPES: DataType[] = [
  // ── PII ──────────────────────────────────────────────────────────────
  ...
  // Configuration is its OWN family: a "source code" clause must not match a
  // package.json or a deployment YAML by prefix (audit G12 — the over-block).
  t("CONFIG", "Configuration files (any)", ["configuration", "configuration file", "configuration files", "config", "config file", "config files", "settings file"], { inputs: ["text", "structured", "code"], transforms: { LIMIT: "supported", REDACT: "supported", DROP_FIELD: "supported" } }),
  t("CONFIG.BUILD", "Build / package manifests", ["build config", "build configuration", "package manifest", "package json", "build file", "lockfile"], { inputs: ["text", "structured", "code"], transforms: { LIMIT: "supported", REDACT: "supported" } }),
  t("CONFIG.APP", "Application configuration", ["app config", "application configuration", "yaml config", "ini file", "properties file", "toml file"], { inputs: ["text", "structured", "code"], transforms: { LIMIT: "supported", REDACT: "supported", DROP_FIELD: "supported" } }),
...
/* ------------------------------------------------------------------ *
 * Registry instance — built-ins plus tenant types.
 * ------------------------------------------------------------------ */
...
export interface CustomTypeSpec {
  ...
  label: string;
  aliases: string[];
  detection: { kind: "edm_index" | "regex" | "dictionary" | "label_map" | "model" };
...
export class DataTypeRegistry {
  private byId = new Map<string, DataType>();
  private aliasIndex = new Map<string, string>();
  readonly version: string;

  constructor(types: DataType[] = BUILTIN_TYPES, version = "1.0.0") {
    this.version = version;
    for (const d of types) this.add(d);
  }

  add(d: DataType): void {
    if (!/^[A-Z][A-Z0-9_]*(\.[A-Za-z0-9_-]+)*$/.test(d.id)) throw new Error(`invalid data type id: ${d.id}`);
    this.byId.set(d.id, d);
    this.aliasIndex.set(norm(d.id.replace(/[._]/g, " ")), d.id);
    for (const a of d.aliases) if (a && !this.aliasIndex.has(a)) this.aliasIndex.set(a, d.id);
  }
  ...
  get(id: string): DataType | undefined { return this.byId.get(id); }
  has(id: string): boolean { return this.byId.has(id); }
  all(): DataType[] { return [...this.byId.values()]; }
  ...
  family(id: string): string { return id.split(".")[0]; }
   ...
   */
  resolve(phrase: string): { id: string; via: "id" | "alias" | "phrase" } | null {
    const raw = phrase.trim();
    if (!raw) return null;
    const upper = raw.toUpperCase().replace(/\s+/g, "_");
    if (this.byId.has(upper)) return { id: upper, via: "id" };
    const n = norm(raw);
    const exact = this.aliasIndex.get(n);
    if (exact) return { id: exact, via: "alias" };
    ...
    for (const [alias, id] of this.aliasIndex) {
      ...
      if (aw.length > words.length) continue;
      for (let i = 0; i + aw.length <= words.length; i++) {
        if (aw.every((w, j) => words[i + j] === w)) {
          if (!best || aw.length > best.len) best = { id, len: aw.length };
          break;
        }
      }
    }
   ...
   */
  resolveAll(phrase: string): Array<{ id: string; via: "id" | "alias" | "phrase"; match: string }> {
    const direct = this.resolve(phrase);
    if (direct && (direct.via === "id" || direct.via === "alias")) return [{ ...direct, match: phrase.trim() }];
    const words = norm(phrase).split(" ").filter(Boolean);
    const stems = words.map(stem);
    const taken = new Array<boolean>(words.length).fill(false);
    const hits: Array<{ id: string; start: number; len: number }> = [];
    ...
    for (const { a, id } of aliases) {
      for (let i = 0; i + a.length <= words.length; i++) {
        if (taken.slice(i, i + a.length).some(Boolean)) continue;
        if (a.every((w, j) => stems[i + j] === w)) {
          hits.push({ id, start: i, len: a.length });
          for (let k = i; k < i + a.length; k++) taken[k] = true;
        }
      }
    }
    hits.sort((x, y) => x.start - y.start);
    const seen = new Set<string>();
    const out: Array<{ id: string; via: "id" | "alias" | "phrase"; match: string }> = [];
    for (const h of hits) {
  ...
  legacyKind(id: string): string | null {
    for (const anc of this.lineage(id)) {
      ...
      if (d?.legacyKind) return d.legacyKind;
    }
    return null;
  }
...
let defaultRegistry: DataTypeRegistry | null = null;
/** The process-wide registry (built-ins + whatever tenant types were added). */
export function dataTypes(): DataTypeRegistry {
  if (!defaultRegistry) defaultRegistry = new DataTypeRegistry();
  return defaultRegistry;
}
/** Test hook: replace the process-wide registry. */
export function setDataTypeRegistry(r: DataTypeRegistry | null): void { defaultRegistry = r; }
